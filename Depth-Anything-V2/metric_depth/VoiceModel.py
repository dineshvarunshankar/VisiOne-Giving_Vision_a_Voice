...
class VoiceGuide:
    def __init__(self, model_name="tts_models/multilingual/multi-dataset/xtts_v2", language="en"):
        ...
        # --- CRITICAL FIX FOR MAC ---
        # XTTS often has stability issues on Apple Silicon GPU (MPS) due to unsupported operations.
        # We must FORCE the audio model to use the CPU for stability on Mac.
        # This does NOT affect YOLO or Depth models (which should stay on GPU/MPS).
        if DEVICE == 'mps':
            self.device = 'cpu'
            print(f"[TTS] Apple Silicon detected. Forcing XTTS to run on CPU (Stable).")
        else:
            # For NVIDIA/CUDA or other supported devices
            self.device = DEVICE 
            print(f"[TTS] Loading xTTS ({model_name}) on {self.device}...")
